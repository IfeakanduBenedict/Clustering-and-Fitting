...
This script performs data cleaning, visualization, clustering, and polynomial fitting on the Spotify dataset.
...
def load_and_clean_data(data):
    ...
    Returns:
        pd.DataFrame: A cleaned DataFrame with irrelevant columns removed and missing values dropped.
    """
    df = pd.read_csv(data)
...
def relational_plot(dataframe, x_var, y_var, color_var=None):
    """
    Create a relational scatter plot for two variables, optionally colored by a third variable.

    Parameters:
        dataframe (pd.DataFrame): The data source.
        x_var (str): The column name for the x-axis.
        y_var (str): The column name for the y-axis.
        color_var (str, optional): The column name for coloring the points. Defaults to None.
...
def plot_heatmap(dataframe):
    """
    Generate a heatmap of the correlation matrix for numeric features in the dataframe.
...
def plot_distribution(dataframe):
    ...
    For numeric columns, display histograms with KDE; for categorical columns, display count plots.
...
def elbow_method(dataframe, features, max_k=10):
    ...
    Returns:
        None. Displays a plot of the sum of squared distances (SSE) for different values of k.
    """
...
def silhouette_method(dataframe, features, max_k=10):
    """
    Use the Silhouette Method to evaluate the quality of clustering for different numbers of clusters.
...
def k_means_clustering(dataframe, features, n_clusters=4):
    """
    Apply K-Means clustering to the dataframe using selected features and add the resulting cluster labels to the dataframe.

    Parameters:
        dataframe (pd.DataFrame): The data source.
        features (list): A list of column names to use for clustering.
        n_clusters (int, optional): The number of clusters to form. Defaults to 4.
...
def predict_with_confidence(poly_model, dataframe, x_feature, y_feature, confidence=0.95):
    """
    Generate predictions with a polynomial model, calculate confidence intervals, and plot the results.
    ...
    plt.fill_between(x_pred, y_pred - ci, y_pred + ci, color='grey', alpha=0.4,
...
def statistical_moments(dataframe, features):
    """
    Calculate and display statistical moments (mean, variance, skewness, kurtosis) for a set of features.
...
def main():
    ...
    Steps:
      ...
      2. Generate various visualizations including relational plots, bar plots, heatmaps, distributions, and pairplots.
      3. Determine optimal clusters using Elbow and Silhouette methods.
      4. Apply K-Means clustering and visualize the results.
      5. Fit a polynomial model to the data and plot predictions with confidence intervals.
    ...
    features_for_clustering = [feat for feat in features_for_clustering if feat in df.columns]
    ...
    if all(col in df.columns for col in ['danceability', 'energy']):
        ...
        predict_with_confidence(poly_model, df, 'danceability', 'energy', confidence=0.95)
    else:
